/**
 * Organization Copilot telemetry — Admin health panel.
 */

import type {
  OrgCopilotHealthCounters,
  OrgCopilotIntent,
} from "@/lib/ai/copilot/org-types";

const counters: OrgCopilotHealthCounters = {
  requests: 0,
  failures: 0,
  totalLatencyMs: 0,
  totalConfidence: 0,
...
export function recordOrgCopilotTelemetry(event: {
  success: boolean;
  latencyMs: number;
  confidence?: number;
...
export function getOrgCopilotTelemetrySnapshot(): OrgCopilotHealthCounters & {
...
export function resetOrgCopilotTelemetryForTests(): void {
  counters.requests = 0;
  counters.failures = 0;
