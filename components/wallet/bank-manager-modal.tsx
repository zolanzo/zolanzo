...
export function BankManagerModal({ isOpen, onClose, onBankAdded }: BankManagerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-[440px] bg-[#0A0F12] border border-white/10 rounded-3xl p-6 shadow-2xl relative text-white space-y-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-400">Link a verified Nigerian bank account for instant withdrawals.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full h-[48px] px-4 rounded-xl bg-zinc-900 border border-zinc-800 focus:border-[#008744] text-white text-xs font-bold focus:outline-none"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-zinc-300">10-Digit Account Number</label>
            <input
              type="text"
              required
              value={accountNumber}
          ...
          {verifiedName && (
            <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-xs flex items-center gap-2">
              <HugeiconsIcon icon={CheckmarkCircle01Icon} size={18} className="text-emerald-400 shrink-0" />
              <div>
                <p className="font-bold text-white">{verifiedName}</p>
                <p className="text-[10px] text-emerald-400">Account verified with NIBSS</p>
              </div>
            </div>
          )}
          ...
          <button
            ...
            className="w-full h-[48px] rounded-xl bg-[#008744] hover:bg-[#00753b] text-white font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shadow-md"
          >
